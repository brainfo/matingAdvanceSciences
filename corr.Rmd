...
dds <- DESeq2::DESeqDataSetFromMatrix(countData = as.matrix(data_distinct),
                              colData = samples,
                              design = ~ condition)
dds.pgc <- DESeqDataSetFromMatrix(countData = as.matrix(data_distinct.pgc),
                              colData = samples.pgc,
                              design = ~ condition)
#colnames(dds.pgc) <- colData(dds.pgc)$condition
dds.nooutlier <- dds[, !grepl("FLUM2_13.5|FLUF2_13.5|CNTF2_10.5|DHTF1_10.5", colnames(dds))]
...
### filtering
```{r filter}
sprintf("original gene number: %s; original sample number: %s", nrow(dds.nooutlier), ncol(dds.nooutlier))
keep <- rowSums(counts(dds.nooutlier) > 0) >= 6
dds.filter <- dds.nooutlier[keep, ]
colnames(dds.filter) <- colData(dds.filter)$condition
## remove mt genes
dds.filter.nomt <- dds.filter[!grepl('^mt-',rownames(dds.filter)),]
colSums(assay(dds.filter.nomt))/colSums(assay(dds.filter))
...
```{r tech}
log_ <- log1p(assay(dds.filter.nomt) * 1000000 / colSums(assay(dds.filter.nomt)))
sampleCors <- cor(log_, method = 'spearman')

embryo_info <- read.csv('samples/sample_info.txt', sep='\t')
embryo_info = mutate(embryo_info, Sex=substr(Samples, 4, 4))
rownames(embryo_info) <- embryo_info$Samples
## subset embryo_info with the same samples as dds
embryo_info = embryo_info[colData(dds.filter.nomt)$sample,]
column_ha = HeatmapAnnotation(time = colData(dds.filter.nomt)$time, condition = colData(dds.filter.nomt)$condition, sex = colData(dds.filter.nomt)$gender)
...
pca = plotPCA(log_dds, intgroup=c('condition','time', 'gender'), returnData=T)
...
dds.e10.filter <- DESeq(dds.e10.filter)
...
restime$gene_group = ifelse(rownames(restime) %in% c(DHT_up_not_FLU, DHT_up_FLU), 'DHT up', ifelse(rownames(restime) %in% c(DHT_down_not_FLU, DHT_down_FLU), 'DHT down', 'DHT invariant'))
time_2plot = data.frame(restime)
table(time_2plot[,c('cutoff','gene_group')])
pal <- c("firebrick3", "dodgerblue3", "black")
pal <- setNames(pal, c("Up", "Down", ""))
fig = plot_ly(data = data.frame(resdht), x = ~log2FoldChange, y = ~logpadj, color = ~cutoff, colors=pal)
fig <- fig%>% layout(
...
## normalization
```{r}
dds.e10.filter <- estimateSizeFactors(dds.pgc.nooutlier)
#sizeFactors(dds)
normalized_counts <- counts(dds.e10.filter, normalized = TRUE)
...
data_dlk1 = ln.counts[c('Tubb1', 'Itga2b'),]
data_risk <- na.omit(data_risk)
data_dlk1 <- na.omit(data_dlk1)
resLong <- reshape(
  data_risk,
  varying = colnames(ln.counts)[1:22],
  direction = "long",
  idvar = c("genes"),
  v.names = "expression"
)
...
n_genes = 2
resLong$condition = rep(dds.pgc.nooutlier$condition, n_genes)
write.table(
  resLong,
  file = "Risk_table.txt",
  append = FALSE,
...
data1 <- ln.counts[ln.counts$genes %in% rownames(dht.cnt.up), ]
data1$gene_group <- 'DHT>CNT'
data2 <- ln.counts[ln.counts$genes %in% rownames(dht.cnt.down), ]
...
coldata = colData(dds.e10.filter)[with(colData(dds.e10.filter), order(condition, gender)),]
column_ha = HeatmapAnnotation(condtion = coldata$condition,
                              sex = coldata$gender)

data_all_zscore_sort = data_all_zscore[
  ,with(colData(dds.e10.filter), order(condition, gender))
]
pdf('DEGs/gene_groups_heatmap_placenta_DHT_CNT.pdf', family = 'ArialMT')
Heatmap(
  ...
  row_split = rep(c('DHT>CNT', 'DHT<CNT'), c(nrow(data1), nrow(data2))),
  row_title_rot = 0,
  row_gap = unit(5, "mm")
)
dev.off()
